 ...
 * @description
 * # speedtest
 * Service in the ndtApp.
 */

angular
    .module('speedtest', [])
    .factory('Speedtest', Speedtest);

Speedtest.$inject = ['$http'];
  ...
  function Speedtest($http) {
    ...
    var Speedtest = {
      ...
      MSG_EXTENDED_LOGIN: MSG_EXTENDED_LOGIN,
      NDT_MESSAGES: NDT_MESSAGES,
      logger: logger,
      checkBrowserSupport: checkBrowserSupport,
      findNdtServer: findNdtServer,
      makeLoginMessage: makeLoginMessage,
      makeNdtMessage: makeNdtMessage,
      parseNdtMessage: parseNdtMessage,
      ConnectionException: ConnectionException,
      UnsupportedBrowser: UnsupportedBrowser,
      TestFailureException: TestFailureException,
      createWebsocket: createWebsocket,
      ndtC2sTest: ndtC2sTest,
      ndtS2cTest: ndtS2cTest,
      ndtMetaTest: ndtMetaTest,
      startTest: startTest,
      reportPassedResults: reportPassedResults,
      getRegionalSpeed: getRegionalSpeed,
      getRecordedTests: getRecordedTests,
     ...
     *
     */
    function getRegionalSpeed(distance, latitude, longitude)
    {
      var url = '/stats/regional/';
      ...
      if (distance)
      {
        ...
        {
           params.point = latitude + ',' + longitude;
        }
      }
     ...
     *
     */
    function reportPassedResults(passed_result, binding_hash, download_rate, latency, upload_rate, latitude, longitude)
    {
        return $http.post('/ndt/web100/', {
          blob: passed_result,
          hash: binding_hash,
          ...
          latitude: latitude,
          longitude: longitude
        }).then(reportPassedResultsSuccessFn, reportPassedResultsErrorFn);
    }

    /**
     * @name reportPassedResultsSuccessFn
     * @desc
     */
    function reportPassedResultsSuccessFn(data, status, headers, config) {
      return data.data.hash;
    }

    /**
     * @name reportPassedResultsErrorFn
     * @desc
     */
 ...
 */
function logger(logMessage, debugging) {
 ...
 */
function makeLoginMessage(desiredTests) {
 ...
 *  websocket.
 */
function makeNdtMessage(messageType, messageContent) {
 ...
 */
function parseNdtMessage(buffer) {
  var i,
    response = [],
 ...
 *    be called.
 */
function ndtMetaTest(ndtSocket) {
  var errorMessage,
    that = this,
    state = 'WAIT_FOR_TEST_PREPARE';
 ...
 **/
function startTest(server, serverPort, serverPath, callbacks, updateInterval) {
	this.server = server;
  ...
  that.logger('Test started.  Waiting for connection to server...');

...
class Base(models.Model):
    ...
    def __str__(self):
        display_name = getattr(self, 'alternate_names', None)
        if display_name:
            return display_name
        return self.name
...
class AbstractRegion(Base):
    ...
    country = models.ForeignKey(CITIES_LIGHT_APP_NAME + '.Country', related_name="%(app_label)s_%(class)s_country")
...
class AbstractCity(Base):
    ...
    region = models.ForeignKey(CITIES_LIGHT_APP_NAME + '.Region', blank=True,
                               null=True, related_name="%(app_label)s_%(class)s_region")
    country = models.ForeignKey(CITIES_LIGHT_APP_NAME + '.Country', related_name="%(app_label)s_%(class)s_country")

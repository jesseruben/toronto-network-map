...
class NDTSerializerSmall(serializers.ModelSerializer):
    class Meta:
        model = NDT
        fields = ('id', 'download_rate', 'upload_rate', 'isp_name', 'rating_general', 'price')

    def create(self, validated_data):
        return NDT.objects.create(**validated_data)

    def update(self, instance, validated_data):
        instance.upload_rate = (instance.upload_rate*float(instance.average_index) + float(validated_data.get('upload_rate', None)))/float(instance.average_index+1)
        instance.download_rate = (instance.download_rate*float(instance.average_index) + float(validated_data.get('download_rate', None)))/float(instance.average_index+1)
        instance.average_index = instance.average_index+1
        instance.save()
        return instance
